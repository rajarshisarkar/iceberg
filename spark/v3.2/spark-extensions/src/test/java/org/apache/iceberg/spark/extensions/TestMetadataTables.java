...
public class TestMetadataTables extends SparkExtensionsTestBase {
  ...
  public void testSnapshotReferencesMetatable() throws Exception {
    ...
    List<Row> testTagProjection = spark
                    .sql("SELECT name,type,snapshot_id,max_reference_age_in_ms,min_snapshots_to_keep FROM " +
                            tableName +
                            ".refs where type='TAG'")
                    .collectAsList();
    ...
    List<Row> testBranchProjection =
        spark.sql("SELECT type,name,max_reference_age_in_ms,snapshot_id FROM " +
                tableName +
                ".refs WHERE name = 'testBranch' AND type='BRANCH'").collectAsList();
    ...
    Assert.assertThrows(IllegalArgumentException.class,
            () -> testBranchProjection.get(0).getAs("min_snapshots_to_keep"));
    Assert.assertThrows(IllegalArgumentException.class,
            () -> testBranchProjection.get(0).getAs("max_snapshot_age_in_ms"));
  }
